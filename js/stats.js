// BANIPAY — stats.js
// NOTE: renderDashboard(), renderTVA(), initRecherche() et
// rechercheGlobale() ont été retirées de ce fichier — elles étaient
// dupliquées avec des versions plus complètes dans factures.js/nav.js, et
// comme ce fichier se charge APRÈS ceux-là, ses versions écrasaient
...
function renderStatsDashboard() {
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    months.push({
      ...
      month: d.getMonth(), year: d.getFullYear(), ca: 0, paye: 0, count: 0
    });
  }
  f.forEach(fac => {
    ...
    if (m) { m.ca += Number(fac.ttc) || 0; if (fac.statut === 'payee') m.paye += Number(fac.ttc) || 0; m.count++; }
  });
