...
async function refuserInvitationEmail(emailEnc, entrepriseId) {
  ...
  try {
    ...
    if (!r.ok) {
      document.body.innerHTML = '<div style="text-align:center;padding:60px;color:#B23A2E;font-family:Arial">❌ Échec — l\'invitation n\'a pas pu être mise à jour. Réessayez ou contactez le support.</div>';
      return;
    }
    ...
    document.body.innerHTML = '<div style="text-align:center;padding:60px;color:#B23A2E;font-family:Arial">❌ Erreur: ' + e.message + '</div>';
  }
}
...
async function afficherBonCommandePublic(bcId, token) {
  try {
    ...
    genDocPDF({
      ...
      paiement: '',
      // FIX (même bug systémique) : garde contre des lignes stockées
      // comme texte JSON brut plutôt qu'un vrai tableau.
      lignes: (typeof bc.lignes === 'string' ? JSON.parse(bc.lignes || '[]') : (bc.lignes || [])),
      note: bc.note || '',
      ht: (typeof bc.lignes === 'string' ? JSON.parse(bc.lignes || '[]') : (bc.lignes||[])).reduce(function(s,l){return s+(l.qte||1)*(l.pu||0);},0),
      tva: 0, ttc: 0,
      devise: 'MAD',
      showPrices: true,
      doc_id: bcId,
...
async function afficherBonLivraisonPublic(blId, token) {
  try {
    ...
    genDocPDF({
      ...
      paiement: '',
      lignes: (typeof bl.lignes === 'string' ? JSON.parse(bl.lignes || '[]') : (bl.lignes||[])).map(function(l) { return { desc: l.desc, qte: l.qte, pu: 0, unite: l.unite||'u' }; }),
