...
const SEUIL_ACTION = 0.3;
const SEUIL_EN_COURS = 0.95;
  ...
  try {
    ...
    for (const eid of ids) {
      try {
        ...
        if (r.ok) { toutesFactures = toutesFactures.concat((await r.json()) || []); }
      } catch(eRpc) {}
    }
    let tousControles = [];
    for (const eid of ids) {
      try {
        ...
        if (r.ok) { tousControles = tousControles.concat((await r.json()) || []); }
      } catch(eRpc) {}
    }
    let tousAchats = [];
    ...
    for (const eid of ids) {
      try {
        ...
        if (r.ok) { tousAchats = tousAchats.concat((await r.json()) || []); }
      } catch(eRpc) {}
      try {
    ...
    CPT.allFactures = [];
    CPT.allControles = [];
    // FIX (grand audit - correctif 2): agregats globaux des achats,
    // manquaient totalement alors qu'ils existent deja par entreprise
    // (inv._achats, inv._controlesAchats). Necessaires pour que le
    // tableau de bord principal et le tri par lettrage/TVA refletent
    // aussi les achats, coherent avec calculerEtat() et les fiches
    // entreprise individuelles (qui, elles, incluaient deja les achats).
    CPT.allAchats = [];
    CPT.allControlesAchats = [];
    CPT.entreprises.forEach(function(inv) {
      CPT.allFactures = CPT.allFactures.concat(inv._factures || []);
      CPT.allControles = CPT.allControles.concat(inv._controles || []);
      CPT.allAchats = CPT.allAchats.concat(inv._achats || []);
      CPT.allControlesAchats = CPT.allControlesAchats.concat(inv._controlesAchats || []);
    });
  ...
  if (kpis) kpis.innerHTML =
    '<div style="padding:16px">' +
      '<div style="background:' + etatColor(etat) + '20;border-radius:12px;padding:10px 14px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between">' +
        ...
        '<span style="font-size:11px;color:#6B5F54">Controle global: ' + txG + '%</span>' +
      '</div>' +
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px">' +
        ...
        '<div style="background:#F7EFDC;border-radius:12px;padding:12px;text-align:center">' +
          ...
          '<div style="font-size:10px;color:#B8860B">Non lettrees</div>' +
        '</div>' +
        '<div style="background:#EDE6F0;border-radius:12px;padding:12px;text-align:center">' +
          ...
          '<div style="font-size:10px;color:#7C5CA6">TVA a verifier</div>' +
        '</div>' +
      '</div>' +
      ...
      '</div>' +
      '<div style="margin-bottom:8px">' +
        '<div style="display:flex;justify-content:space-between;font-size:11px;color:#6B5F54;margin-bottom:4px"><span>Verification TVA</span><span style="font-weight:700">' + txT + '%</span></div>' +
      ...
      '</div>' +
      '<div>' +
        '<div style="display:flex;justify-content:space-between;font-size:11px;color:#6B5F54;margin-bottom:4px"><span>Controle global</span><span style="font-weight:700">' + txG + '%</span></div>' +
...
function creerFiltreCpt() {
  const btns = [
    { label: '☐ Non lettrees', key: 'lettrage', val: 'false', bg: '#F7EFDC', color: '#B8860B' },
    { label: '☐ TVA non verifiee', key: 'tva', val: 'false', bg: '#EDE6F0', color: '#7C5CA6' },
...
function renderCptFactures() {
  ...
  if (CPT.filtres.moisFactures) { f = f.filter(function(fac) { return (fac.date_emission || '').substring(0, 7) === CPT.filtres.moisFactures; }); }
  if (CPT.filtres.rechercheClient) { const q = CPT.filtres.rechercheClient.toLowerCase(); f = f.filter(function(fac) { return (fac.client || '').toLowerCase().includes(q); }); }
  if (CPT.filtres.statutFacture) { f = f.filter(function(fac) { return fac.statut === CPT.filtres.statutFacture; }); }
  ...
  const statutLabel = { payee:'Payee', attente:'En attente', retard:'En retard', brouillon:'Brouillon', envoyee:'Envoyee', annulee:'Annulee' };
  ...
  list.innerHTML =
    ...
    '</div>' +
    '<div style="padding:6px 16px;background:#EEF3E4;font-size:10px;color:#55702E;border-bottom:1px solid #E3DCCF">dont ' + fmt(caPaye) + ' MAD encaisse(s)</div>' +
...
async function toggleLettrageRapide(factureId, btn) {
  ...
  ajouterHistorique(isLettree ? 'Lettrage retire — ' : 'Lettrage effectue — ', factureId);
  showToast('Lettrage mis a jour', 'success');
  const inv = CPT.entreprises.find(function(e) { return e.entreprise_id === CPT.currentEntrepriseId; });
...
async function toggleTVARapide(factureId, btn) {
  ...
  ajouterHistorique(isTVA ? 'TVA non verifiee — ' : 'TVA verifiee — ', factureId);
  showToast('TVA mise a jour', 'success');
}
...
function renderCptAchats() {
  ...
  if (CPT.filtres.moisAchats) { achats = achats.filter(function(a) { return (a.date_achat || '').substring(0, 7) === CPT.filtres.moisAchats; }); }
  if (CPT.filtres.statutAchat) { achats = achats.filter(function(a) { return a.statut === CPT.filtres.statutAchat; }); }
  if (CPT.filtres.rechercheFournisseur) { const q = CPT.filtres.rechercheFournisseur.toLowerCase(); achats = achats.filter(function(a) { return (a.fournisseur || '').toLowerCase().includes(q); }); }
  const statutBg = { payee: '#EEF3E4', attente: '#F7EFDC' };
  const statutColor = { payee: '#55702E', attente: '#B8860B' };
  const statutLabel = { payee: 'Payee', attente: 'En attente' };
  ...
  list.innerHTML =
    ...
    '</div>' +
    '<div style="padding:6px 16px;background:#F5E4E1;font-size:10px;color:#8E2E24;border-bottom:1px solid #E0B6AC">dont TVA deductible : ' + fmt(totalTvaAchats) + ' MAD</div>' +
    creerFiltreMoisCpt('moisAchats', CPT.currentAchats, 'date_achat') +
    creerFiltreRechercheCpt('rechercheFournisseur', CPT.filtres.rechercheFournisseur, 'Rechercher un fournisseur...', 'renderCptAchats') +
    filtreStatutHtml +
    '<div style="display:grid;grid-template-columns:1fr 34px 34px 34px 34px;padding:8px 16px;background:#F8FAFC;border-bottom:1px solid #F1EEE8">' +
  ...
  overlay.innerHTML =
    ...
    '<div style="margin:0 14px 14px;background:#fff;border-radius:14px;padding:14px;border:1px solid #E3DCCF">' +
      ...
      '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0">' +
        '<div><div style="font-size:12px;font-weight:600">Rapprochement bancaire</div><div style="font-size:10px;color:#9C9186">' + ((a.transactions_bancaires_liees||[]).length ? (a.transactions_bancaires_liees||[]).length + ' transaction(s) liée(s)' : 'Aucune transaction liée') + '</div></div>' +
        '<button class="btn-rapprocher-achat-ov" onclick="ouvrirRapprochementDepuisFacture(\'' + a.id + '\',\'achat\')" style="padding:7px 12px;border-radius:8px;border:none;background:' + ((a.transactions_bancaires_liees||[]).length ? '#1F6F72' : '#E9F4F3') + ';color:' + ((a.transactions_bancaires_liees||[]).length ? '#fff' : '#1F6F72') + ';font-size:11px;font-weight:600;cursor:pointer;font-family:inherit">🏦 Rapprocher</button>' +
      '</div>' +
    '</div>' +
...
async function toggleControleAchatRapide(achatId, champ, btn) {
  ...
  try {
    ...
    if (!resp.ok) { showToast('❌ Erreur lors de la mise a jour', 'error'); return; }
    const resultat = await resp.json();
    if (!resultat) return;
    ...
    showToast(champ === 'lettre' ? 'Lettrage mis a jour' : 'TVA mise a jour', 'success');
  } catch(e) { showToast('❌ Erreur lors de la mise a jour', 'error'); }
}
...
function renderCptDevis() {
  ...
  if (!d.length) { list.innerHTML = '<div class="empty"><div class="empty-ico">📝</div><div class="empty-title">Aucun devis</div></div>'; return; }
  const statutColor = { envoye: '#B8860B', accepte: '#6E8F4E', refuse: '#B23A2E', brouillon: '#9C9186' };
  const statutLabel = { envoye: 'Envoye', accepte: 'Accepte', refuse: 'Refuse', brouillon: 'Brouillon' };
  ...
  list.innerHTML = '<div style="padding:16px">' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">' +
      '<div style="background:#EDE6F0;border-radius:12px;padding:12px">' +
        '<div style="font-size:11px;font-weight:600;color:#7C5CA6">TVA collectee</div>' +
      ...
      '<div style="background:#F5E4E1;border-radius:12px;padding:12px">' +
        '<div style="font-size:11px;font-weight:600;color:#B23A2E">TVA deductible (achats)</div>' +
    ...
    '<div style="background:#241F1B;border-radius:12px;padding:14px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center">' +
      '<div style="font-size:12px;font-weight:600;color:#fff">TVA nette a payer</div>' +
    ...
    Object.keys(byMonth).sort().reverse().map(function(m) {
      ...
      return '<div style="padding:12px 0;border-bottom:1px solid #EAE4DA">' +
        '<div style="display:flex;justify-content:space-between">' +
          ...
          '<div style="text-align:right"><div style="font-size:11px;color:#6B5F54">Collectee: ' + fmt(d.tva) + ' · Deductible: ' + fmt(d.tvaAchats) + '</div><div style="font-size:13px;font-weight:700;color:' + (net >= 0 ? '#7C5CA6' : '#B23A2E') + '">Nette: ' + fmt(net) + ' MAD</div></div>' +
        '</div>' +
      '</div>';
    }).join('') +
    '<div style="display:flex;gap:8px;margin-top:16px">' +
        '<button onclick="declarerTVAMois()" style="flex:1;padding:12px;background:#6E8F4E;color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">✅ Declarer TVA du mois</button>' +
  ...
  list.innerHTML = '<div style="padding:16px">' +
    ...
    (ex.type !== 'devis' ? (
      ...
      '<select onchange="CPT.extraction.statut=this.value;renderCptExtraction()" style="width:100%;padding:8px;border:1.5px solid #E3DCCF;border-radius:8px;font-size:12px;font-family:inherit;background:#F1EEE8;margin-bottom:12px">' +
        ...
        (ex.type === 'factures'
          ? ['envoyee','attente','payee','retard','annulee'].map(function(s){return '<option value="'+s+'"'+(s===ex.statut?' selected':'')+'>'+({envoyee:'Envoyee',attente:'En attente',payee:'Payee',retard:'En retard',annulee:'Annulee'}[s])+'</option>';}).join('')
          : ['attente','payee'].map(function(s){return '<option value="'+s+'"'+(s===ex.statut?' selected':'')+'>'+({attente:'En attente',payee:'Payee'}[s])+'</option>';}).join('')
        ) +
      '</select>'
    ) : '') +
    '<div style="background:#F1EEE8;border-radius:10px;padding:12px;margin-bottom:14px;font-size:12px;color:#6B5F54">' + calculerApercuExtractionCpt() + ' ligne(s) correspondent a ce filtre</div>' +
...
function calculerApercuExtractionCpt() { return _filtrerExtractionCpt().length; }
function extraireCSVCpt() {
  const ex = CPT.extraction;
  const lignes = _filtrerExtractionCpt();
  if (!lignes.length) { showToast('Aucune ligne a extraire', 'error'); return; }
  let headers, rows;
  if (ex.type === 'factures') {
    headers = ['Reference','Client','Date','Echeance','HT','TVA','TTC','Statut'];
    ...
    headers = ['Fournisseur','Reference','Date','HT','TVA','TTC','Statut'];
    ...
    headers = ['Reference','Client','Date','Validite','HT','TVA','TTC','Statut'];
...
function renderCptDocuments() {
  ...
  const statutLabelBC = { brouillon: 'Brouillon', envoye: 'Envoye', confirme: '✅ Confirme', refuse: '❌ Refuse' };
...
function exportCptTVA() {
  ...
  if (!payees.length) { showToast('Aucune facture payee', 'error'); return; }
  const headers = ['Mois', 'Reference', 'Client', 'HT', 'TVA', 'TTC'];
  ...
  const csv = [headers].concat(rows).map(function(r) { return r.map(function(v) { return '"' + String(v).replace(/"/g, '""') + '"'; }).join(','); }).join('\n');
  const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  ...
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  setTimeout(function() { URL.revokeObjectURL(url); }, 3000);
  showToast('✅ Export TVA telecharge !', 'success');
}

function quitterComptable() { if (confirm('Se deconnecter ?')) { sb.logout(); goScreen('auth'); } }
...
async function renderVueATraiter() {
  ...
  CPT.entreprises.forEach(function(inv) {
    ...
    if (nonLettrees.length) { items.push({ entreprise: nomAffiche, type: 'lettrage', count: nonLettrees.length, eid: inv.entreprise_id, label: nonLettrees.length + ' facture(s) non lettree(s)', color: '#B8860B', bg: '#F7EFDC' }); }
    ...
    if (tvaKo.length) { items.push({ entreprise: nomAffiche, type: 'tva', count: tvaKo.length, eid: inv.entreprise_id, label: tvaKo.length + ' TVA non verifiee(s)', color: '#7C5CA6', bg: '#EDE6F0' }); }
    const remsOuvertes = rems.filter(function(r) { return r.statut === 'ouverte'; });
    if (remsOuvertes.length) { items.push({ entreprise: nomAffiche, type: 'remarque', count: remsOuvertes.length, eid: inv.entreprise_id, label: remsOuvertes.length + ' remarque(s) ouverte(s)', color: '#B23A2E', bg: '#F5E4E1' }); }
  });
  ...
  list.innerHTML =
    '<div style="padding:16px">' +
      '<div style="font-size:14px;font-weight:700;color:#2A2420;margin-bottom:12px">📥 A traiter</div>' +
      '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:20px">' +
        '<div style="background:#F7EFDC;border-radius:12px;padding:12px;text-align:center"><div style="font-size:22px;font-weight:800;color:#B8860B">' + totalLettrage + '</div><div style="font-size:10px;color:#B8860B">Non lettrees</div></div>' +
        '<div style="background:#EDE6F0;border-radius:12px;padding:12px;text-align:center"><div style="font-size:22px;font-weight:800;color:#7C5CA6">' + totalTVA + '</div><div style="font-size:10px;color:#7C5CA6">TVA a verifier</div></div>' +
        '<div style="background:#F5E4E1;border-radius:12px;padding:12px;text-align:center"><div style="font-size:22px;font-weight:800;color:#B23A2E">' + totalRem + '</div><div style="font-size:10px;color:#B23A2E">Remarques</div></div>' +
      '</div>' +
      (items.length ?
        ...
        items.map(function(item) {
          return '<div style="background:#fff;border-radius:12px;padding:14px;border:1px solid ' + item.bg.replace('F', 'E') + ';margin-bottom:8px;display:flex;align-items:center;justify-content:space-between">' +
            '<div><div style="font-size:12px;font-weight:700">' + escapeHTML(item.entreprise) + '</div><div style="font-size:11px;color:' + item.color + ';font-weight:600;margin-top:2px">' + item.label + '</div></div>' +
        ...
        '<div style="text-align:center;padding:40px;color:#9C9186"><div style="font-size:40px;margin-bottom:12px">✅</div><div style="font-size:14px;font-weight:600">Tout est a jour !</div></div>'
      ) +
    '</div>';
  if (list.dataset.clickBound !== '1') {
    list.dataset.clickBound = '1';
...
function ajouterHistorique(action, factureId) {
  ...
  CPT.historique.unshift({
    date: new Date().toLocaleDateString('fr-FR') + ' a ' + new Date().toLocaleTimeString('fr-FR', {hour:'2-digit',minute:'2-digit'}),
    action: action + ref,
    comptable: nom
  });
  ...
  list.innerHTML = '<div style="padding:16px">' +
    ...
    (hist.length ? hist.map(function(h) {
      return '<div style="display:flex;gap:12px;padding:10px 0;border-bottom:1px solid #F1EEE8;align-items:flex-start">' +
        ...
        '<div><div style="font-size:12px;font-weight:600;color:#2A2420">' + escapeHTML(h.action) + '</div><div style="font-size:10px;color:#9C9186;margin-top:2px">' + h.date + ' · ' + escapeHTML(h.comptable) + '</div></div>' +
      '</div>';
    }).join('') : '<div style="text-align:center;color:#9C9186;padding:40px">Aucune action enregistree</div>') +
  '</div>';
}
...
async function retirerEntrepriseComptable(invitationId, nomEntreprise) {
  if (!confirm('Retirer "' + nomEntreprise + '" de votre portefeuille ? Vous perdrez l acces a ses documents.')) return;
  try {
    ...
    showToast('✅ ' + nomEntreprise + ' retiree', 'success');
...
function trierEntreprises(critere) {
  ...
  const sorted = CPT.entreprises.slice().sort(function(a, b) {
    ...
    if (critere === 'lettrage') { const txA = txLettrage(a); const txB = txLettrage(b); return txA - txB; }
    if (critere === 'tva') { const txA = txTVA(a); const txB = txTVA(b); return txA - txB; }
...
// FIX (correctif - grand audit): txLettrage()/txTVA() ne comptaient que
// les factures de vente (inv._factures/inv._controles), alors que le tri
// s'appuie dessus pour classer "par lettrage"/"par TVA" - incoherent avec
// calculerEtat() et les cartes affichees, qui incluent deja les achats
// depuis un correctif precedent. Sans ce correctif, trier "par lettrage"
// classait les entreprises sur un pourcentage different de celui
// reellement affiche sur leur carte.
function txLettrage(inv) {
  const f = inv._factures || []; const a = inv._achats || [];
  const c = inv._controles || []; const cA = inv._controlesAchats || [];
  const total = f.length + a.length;
  if (!total) return 1;
  const lettres = c.filter(function(x){return x.lettre;}).length + cA.filter(function(x){return x.lettre;}).length;
  return lettres / total;
}
function txTVA(inv) {
  const f = inv._factures || []; const a = inv._achats || [];
  const c = inv._controles || []; const cA = inv._controlesAchats || [];
  const total = f.length + a.length;
  if (!total) return 1;
  const tvaOk = c.filter(function(x){return x.tva_verifie;}).length + cA.filter(function(x){return x.tva_verifie;}).length;
  return tvaOk / total;
}
...
async function ouvrirGestionEntreprises() {
  ...
  const statutLabel = { en_attente:'⏳ En attente', acceptee:'✅ Active', refusee:'❌ Refusee', revoquee:'🚫 Revoquee', archivee:'📦 Archivee' };
  box.innerHTML = '<div style="width:40px;height:4px;background:#E3DCCF;border-radius:2px;margin:0 auto 20px"></div>' +
    ...
    invites.map(function(inv) {
      ...
      return '<div style="background:#fff;border-radius:12px;border:1px solid #EAE4DA;padding:14px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center">' +
        '<div><div style="font-size:13px;font-weight:600">' + escapeHTML(nomAffiche) + '</div><div style="font-size:11px;margin-top:2px;color:' + (statutColor[inv.statut] || '#6B5F54') + ';font-weight:600">' + (statutLabel[inv.statut] || inv.statut) + '</div></div>' +
        (inv.statut === 'acceptee' ?
  ...
  box.addEventListener('click', function(e) {
    ...
    if (btn) {
      ...
      if (action === 'archiver' && confirm('Archiver cette entreprise ?')) { mettreAJourInvitation(invId, 'archivee'); }
      else if (action === 'revoquer' && confirm('Annuler cette invitation ?')) { mettreAJourInvitation(invId, 'revoquee'); }
    }
  });
}
...
async function envoyerInvitationEntreprise() {
  ...
  if (saisie.includes('@') && !saisie.includes('http') && !saisie.includes('?')) {
    ...
    if (!emailEnt) { showToast('Aucune entreprise trouvee avec ce nom — essayez son email ou son lien Zelto', 'error'); return; }
  }
  try {
...
async function mettreAJourInvitation(invId, statut) {
  try {
    ...
    if (!r.ok) { showToast('❌ Erreur lors de la mise a jour', 'error'); return; }
...
function switchCptNav(tab) {
  ['dashboard','entreprises','traiter','historique','activite','notifs'].forEach(function(t) {
    ...
    if (!btn) return;
    if (t === tab) { btn.style.background = 'rgba(255,255,255,0.9)'; btn.style.color = '#1F6F72'; }
    else { btn.style.background = 'rgba(255,255,255,0.15)'; btn.style.color = 'rgba(255,255,255,0.7)'; }
  });
  const content = el('cpt-main-content');
  if (!content) return;
  if (tab === 'dashboard') {
    // FIX (correctif - grand audit): les indicateurs du tableau de bord
    // principal ne comptaient toujours que les factures de vente
    // (CPT.allFactures/CPT.allControles), pas coherent avec calculerEtat()
    // et les fiches entreprise, qui incluent deja les achats. Les memes
    // agregats globaux pour les achats (CPT.allAchats/CPT.allControlesAchats),
    // desormais construits dans loadComptableApp(), sont utilises ici.
    const idsFacturesExistantes = (CPT.allFactures || []).map(function(fac) { return String(fac.id); });
    const idsAchatsExistants = (CPT.allAchats || []).map(function(a) { return String(a.id); });
    const controlesValides = (CPT.allControles || []).filter(function(c) { return idsFacturesExistantes.includes(String(c.facture_id)); });
    const controlesAchatsValides = (CPT.allControlesAchats || []).filter(function(c) { return idsAchatsExistants.includes(String(c.facture_id)); });
    const totalFac = (CPT.allFactures || []).length;
    const totalAch = (CPT.allAchats || []).length;
    const totalDocs = totalFac + totalAch;
    const lettresTotal = controlesValides.filter(function(c){return c.lettre;}).length + controlesAchatsValides.filter(function(c){return c.lettre;}).length;
    const tvaOkTotal = controlesValides.filter(function(c){return c.tva_verifie;}).length + controlesAchatsValides.filter(function(c){return c.tva_verifie;}).length;
    const consultesTotal = controlesValides.filter(function(c){return c.consulte;}).length + controlesAchatsValides.filter(function(c){return c.consulte;}).length;
    ...
    content.innerHTML =
      ...
      '<div style="padding:0 16px 4px;display:flex;justify-content:space-between;align-items:center">' +
        ...
        '<div style="display:flex;gap:6px">' +
          '<button class="btn-tri-action" style="font-size:10px;padding:4px 8px;border:none;border-radius:8px;background:#FBF0DA;color:#1F6F72;cursor:pointer;font-family:inherit;font-weight:600">Priorite</button>' +
    ...
    const tmp = document.createElement('div'); tmp.id = 'cpt-ent-content'; content.innerHTML = ''; content.appendChild(tmp); renderVueATraiter();
  } else if (tab === 'historique') {
    const tmp = document.createElement('div'); tmp.id = 'cpt-ent-content'; content.innerHTML = ''; content.appendChild(tmp); renderHistorique();
  } else if (tab === 'activite') {
    const tmp = document.createElement('div'); tmp.id = 'cpt-ent-content'; content.innerHTML = ''; content.appendChild(tmp); renderActiviteClients();
  } else if (tab === 'notifs') {
...
function renderComptableDashboard() {
  ...
  chargerNotificationsComptable();
  CPT.entreprises.forEach(function(inv) { inv._remarques = inv._remarques || []; });
...
async function basculerModeEntreprise() {
  ...
  if (visiteEntrepriseClient) { STATE.entrepriseId = CPT.currentEntrepriseId; }
  else { STATE.entrepriseId = null; }
...
async function renderActiviteClients() {
  const list = el('cpt-ent-content');
  if (!list) return;
  list.innerHTML = '<div style="padding:16px"><div style="font-size:14px;font-weight:700;color:#2A2420;margin-bottom:14px">🔴 Activite recente</div><div style="text-align:center;padding:20px;color:#9C9186">⏳ Chargement...</div></div>';
  try {
    const ids = (CPT.entreprises || []).map(function(e) { return e.entreprise_id; });
    if (!ids.length) { list.innerHTML = '<div class="empty"><div class="empty-ico">📡</div><div class="empty-title">Aucune entreprise</div></div>'; return; }
    ...
    if (!events.length) { list.innerHTML = '<div class="empty"><div class="empty-ico">📡</div><div class="empty-title">Aucune activite recente</div><div>Rien de nouveau ces 7 derniers jours</div></div>'; return; }
    const statutColor = { payee: '#6E8F4E', attente: '#B8860B', retard: '#B23A2E', brouillon: '#9C9186', envoyee: '#C9971F', accepte: '#6E8F4E', refuse: '#B23A2E' };
    const statutLabel = { payee: 'Payee', attente: 'En attente', retard: 'En retard', brouillon: 'Brouillon', envoyee: 'Envoyee', accepte: 'Accepte', refuse: 'Refuse' };
    list.innerHTML = '<div style="padding:16px">' +
      '<div style="font-size:14px;font-weight:700;color:#2A2420;margin-bottom:4px">📡 Activite recente</div>' +
      '<div style="font-size:11px;color:#9C9186;margin-bottom:14px">7 derniers jours · ' + events.length + ' evenement(s)</div>' +
        ...
        return '<div style="display:flex;gap:12px;padding:12px 0;border-bottom:1px solid #F1EEE8;align-items:flex-start">' +
          ...
          '<div style="flex:1">' +
            '<div style="display:flex;justify-content:space-between;align-items:flex-start">' +
              '<div><div style="font-size:12px;font-weight:700;color:#2A2420">' + escapeHTML(ev.label) + '</div><div style="font-size:11px;color:#6B5F54;margin-top:1px">' + escapeHTML(ev.entreprise) + ' · ' + escapeHTML(ev.sous) + '</div></div>' +
              '<div style="text-align:right;flex-shrink:0"><div style="font-size:12px;font-weight:700;color:#2A2420">' + ev.montant + '</div><div style="font-size:9px;color:' + (statutColor[ev.statut] || '#9C9186') + ';font-weight:600">' + (statutLabel[ev.statut] || ev.statut || '') + '</div></div>' +
            '</div>' +
    ...
    '</div>';
  } catch(e) { list.innerHTML = '<div style="text-align:center;padding:40px;color:#B23A2E">Erreur: ' + e.message + '</div>'; }
}

async function declarerTVAMois(mois) {
  if (!mois) { const now = new Date(); mois = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0'); }
  if (!confirm('Declarer la TVA verifiee pour ' + mois + ' ? L entreprise sera notifiee.')) return;
  const nom = sb.user?.user_metadata?.nom || sb.user?.email?.split('@')[0] || 'Comptable';
  try {
    const facsMois = (CPT.currentFactures || []).filter(function(f) { return (f.date_emission || '').startsWith(mois); });
    for (let i = 0; i < facsMois.length; i++) {
      const fac = facsMois[i];
      await sauvegarderControle(fac.id, { tva_verifie: true, tva_verifie_at: new Date().toISOString(), tva_verifie_par: nom });
    }
    const tvaTotale = facsMois.filter(function(f) { return f.statut === 'payee'; }).reduce(function(s, f) { return s + (Number(f.tva) || 0); }, 0);
    const inv = CPT.entreprises.find(function(e) { return e.entreprise_id === CPT.currentEntrepriseId; });
    ...
    await fetch(SUPABASE_URL + '/rest/v1/rpc/envoyer_notification', {
      ...
      body: JSON.stringify({
        ...
        p_type: 'tva_declaree',
        p_titre: 'TVA verifiee — ' + mois,
        p_corps: 'Votre comptable ' + nom + ' a verifie la TVA de ' + mois + '. TVA collectee : ' + fmt(tvaTotale) + ' MAD sur ' + facsMois.length + ' facture(s).'
      })
    });
    ajouterHistorique('TVA declaree — ' + mois + ' (' + facsMois.length + ' factures)', '');
    showToast('✅ TVA ' + mois + ' declaree et entreprise notifiee !', 'success');
    renderCptTVA();
  } catch(e) { showToast('Erreur: ' + e.message, 'error'); }
}
...
async function renderCptReleves() {
  ...
  try {
    ...
    if (!resp.ok) { list.innerHTML = '<div style="text-align:center;padding:40px;color:#B23A2E">Erreur de chargement</div>'; return; }
    ...
    if (!releves.length) { list.innerHTML = '<div class="empty"><div class="empty-ico">🏦</div><div class="empty-title">Aucun releve</div><div>Aucun releve partage</div></div>'; return; }
    list.innerHTML = '<div style="padding:16px">' +
    ...
    list.addEventListener('click', async function handler(ev) {
      ...
      if (badge && badge.textContent !== 'Vu') {
        ...
        if (r.ok) { badge.textContent = 'Vu'; badge.style.background = '#EEF3E4'; badge.style.color = '#6E8F4E'; showToast('Releve marque comme vu', 'success'); }
        else { showToast('Erreur : ' + r.status, 'error'); }
        return;
      }
...
async function ouvrirFactureComptable(factureId) {
  ...
  overlay.innerHTML =
    '<div style="background:linear-gradient(135deg,#241F1B,#1F6F72);padding:14px 20px;display:flex;align-items:center;gap:12px;position:sticky;top:0;z-index:10">' +
      ...
      '<div style="flex:1"><div style="font-size:14px;font-weight:700;color:#fff">' + escapeHTML(fac.ref || '') + '</div><div style="font-size:11px;color:rgba(255,255,255,0.5)">' + escapeHTML(fac.client || '') + '</div></div>' +
...
async function sauvegarderControle(factureId, data) {
  ...
  try {
    const resp = await fetch(SUPABASE_URL + '/rest/v1/rpc/toggle_controle_facture', {
      method: 'POST',
      headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + sb.token, 'Content-Type': 'application/json' },
      body: JSON.stringify({ p_facture_id: String(factureId), p_entreprise_id: CPT.currentEntrepriseId, p_champ: champ, p_valeur: valeur })
    });
    if (!resp.ok) {
      if (resp.status === 404) { showToast('⛔ Fonction non disponible — contacte le support', 'error'); }
      else { showToast('❌ Erreur lors de l enregistrement', 'error'); }
      return;
    }
    const resultat = await resp.json();
    if (!resultat) { showToast('⚠️ Aucune confirmation recue — verifie en base', 'error'); return; }
    ...
    if (local) { Object.assign(local, resultat); } else { CPT.currentControles.push(resultat); }
    const inv = CPT.entreprises.find(function(e) { return e.entreprise_id === CPT.currentEntrepriseId; });
    if (inv) {
      if (!inv._controles) inv._controles = [];
      const lc = inv._controles.find(function(c3) { return String(c3.facture_id) === String(factureId); });
      if (lc) { Object.assign(lc, resultat); } else { inv._controles.push(resultat); }
      inv._etat = calculerEtat(inv);
    }
    ...
    if (globalCtrl) { Object.assign(globalCtrl, resultat); } else { CPT.allControles.push(resultat); }
...
async function toggleLettrage(factureId) {
  ...
  if (ctrl.lettre) {
    ...
    ajouterHistorique('Lettrage retire — ', factureId);
    showToast('Lettrage retire', 'success');
  } else {
    ...
    ajouterHistorique('Lettrage effectue — ', factureId);
    showToast('☑ Lettrage enregistre', 'success');
  }
}
async function toggleTVA(factureId) {
  ...
  if (ctrl.tva_verifie) {
    ...
    if (btn) { btn.textContent = '☐'; btn.style.borderColor = '#E3DCCF'; btn.style.background = '#fff'; }
    ajouterHistorique('TVA non verifiee — ', factureId);
    showToast('TVA non verifiee', 'success');
  } else {
    const now = new Date().toISOString();
    ...
    ajouterHistorique('TVA verifiee — ', factureId);
    showToast('☑ TVA verifiee', 'success');
  }
}
...
async function sauvegarderEditionComptable() {
  ...
  try {
    await fetch(SUPABASE_URL + '/rest/v1/profils_comptable', {
      method: 'POST',
      headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + sb.token, 'Content-Type': 'application/json', 'Prefer': 'resolution=merge-duplicates,return=minimal' },
      body: JSON.stringify({ id: sb.user.id, email: sb.user.email, nom: nom, cabinet: el('ec-cabinet')?.value.trim() || '', tel: el('ec-tel')?.value.trim() || '', specialite: el('ec-specialite')?.value.trim() || '' })
    });
    closeAllModals();
...
function renderComptableProfil() {
  ...
  if (infos) {
    const rows = [['Nom', nom], ['Cabinet', cabinet], ['Email', email], ['Specialite', meta.specialite || ''], ['Telephone', meta.tel || '']].filter(function(r) { return r[1]; });
    infos.innerHTML = rows.map(function(r) {
      return '<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #F1EEE8;font-size:12px"><span style="color:#9C9186">' + r[0] + '</span><span style="font-weight:600">' + escapeHTML(String(r[1])) + '</span></div>';
    }).join('');
  }
...
function partagerProfilComptable() {
  ...
  if (navigator.share) {
    navigator.share({ title: nom + ' — Zelto Comptable', text: 'Voici le profil de votre comptable Zelto : ' + nom, url: lien })
      .catch(function() { navigator.clipboard?.writeText(lien); showToast('Lien copie !', 'success'); });
  } else { navigator.clipboard?.writeText(lien); showToast('✅ Lien copie !', 'success'); }
}
...
async function envoyerInvitationDepuisProfil() {
  ...
  try {
    ...
    await fetch(SUPABASE_URL + '/rest/v1/rpc/envoyer_notification', {
      method: 'POST',
      headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + SUPABASE_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify({ p_user_id: sb.user?.id, p_destinataire_email: emailEnt, p_type: 'invitation_comptable', p_titre: 'Invitation de votre comptable', p_corps: nomCpt + ' souhaite acceder a vos documents Zelto en tant que comptable.', p_meta: JSON.stringify({ comptable_email: emailCpt, nom_comptable: nomCpt }) })
    });
    if (document.getElementById('cpt-invite-email')) document.getElementById('cpt-invite-email').value = '';
    showToast('✅ Invitation envoyee ! L entreprise sera notifiee.', 'success');
  } catch(e) { showToast('Erreur: ' + e.message, 'error'); }
}

function basculerModeDevis() {
  CPT.modeEntreprise = true;
  if (!STATE.devis || !STATE.devis.length) { loadAll().then(function() { appendModeBanner(); goScreen('devis-list'); }); }
  else { appendModeBanner(); goScreen('devis-list'); }
}
...
async function chargerNotificationsComptable() {
  ...
  try {
    ...
    for (const inv of invitations) {
      ...
      try {
        const respProfil = await fetch(SUPABASE_URL + '/rest/v1/profils_entreprise?id=eq.' + inv.entreprise_id + '&select=raison,secteur,ville,tel', { headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + sb.token } });
  ...
  } catch(e) { return { invitations: [], notifs: [] }; }
}
...
async function renderNotificationsComptable() {
  ...
  content.innerHTML =
    '<div style="padding:16px">' +
        ...
        invitations.map(function(inv) {
          ...
          return '<div style="background:#FBF0DA;border-radius:14px;padding:16px;margin-bottom:10px;border:1px solid #E8D9AE" id="inv-cpt-carte-' + inv.id + '">' +
            '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">' +
              ...
              '<div><div style="font-size:13px;font-weight:700">' + escapeHTML(p.raison || inv.entreprise_email || '') + '</div>' +
      ...
      (notifs.length ? '<div style="font-size:12px;font-weight:700;color:#9C9186;text-transform:uppercase;margin:14px 0 10px">Autres notifications</div><div id="cpt-autres-notifs"></div>' : '') +
      (!invitations.length && !notifs.length ? '<div style="text-align:center;padding:40px;color:#9C9186"><div style="font-size:40px;margin-bottom:12px">✅</div><div style="font-size:14px;font-weight:600">Aucune notification</div></div>' : '') +
    '</div>';
  const zoneAutres = document.getElementById('cpt-autres-notifs');
  if (zoneAutres && typeof htmlListeNotifications === 'function') {
    const enveloppees = notifs.map(function(n) { return { type: 'info', icon: '🔔', title: n.titre || '', body: n.corps || '', id: n.id, raw: n }; });
  ...
  if (content.dataset.notifClickBound !== '1') {
    ...
    content.addEventListener('click', async function(e) {
      ...
      if (btnAccept) { await accepterInvitationComptable(btnAccept.dataset.id, btnAccept.dataset.eid); return; }
      const btnReject = e.target.closest('.btn-reject-inv');
      if (btnReject) { await refuserInvitationComptable(btnReject.dataset.id); return; }
      ...
      if (btnPlusTard) {
        ...
        showToast('Vous pourrez decider plus tard — l invitation reste en attente', 'default');
      }
    });
  }
}
async function accepterInvitationComptable(invId, entrepriseId) {
  try {
    ...
    if (entrepriseId) { await ajouterEntrepriseCommeClient(entrepriseId); }
    showToast('✅ Invitation acceptee ! Entreprise ajoutee a vos clients.', 'success');
    await loadComptableApp();
    switchCptNav('dashboard');
  } catch(e) { showToast('Erreur: ' + e.message, 'error'); }
}
...
function voirFactureComptable(facId) {
  try {
    const fac = (CPT.currentFactures || []).find(function(f) { return String(f.id) === String(facId); });
    if (!fac) { showToast('❌ Facture introuvable', 'error'); return; }
    ouvrirPDFComptable(fac, CPT.currentProfil || {});
    setTimeout(function() { attacherControlsToViewer(facId); }, 300);
  } catch(e) { showToast('❌ Erreur lors de l ouverture de la facture', 'error'); }
}

function ouvrirPDFComptable(fac, profil) {
  const lignes = typeof fac.lignes === 'string' ? JSON.parse(fac.lignes || '[]') : (fac.lignes || []);
  genDocPDF({
    type: 'FACTURE', ref: fac.ref, color: '#C9971F', emetteur: profil,
    destinataire: { nom: fac.client, chantier: fac.chantier },
    date: fac.date_emission, echeance: fac.echeance, paiement: fac.paiement, statut: fac.statut,
    lignes: lignes, note: fac.note || '', ht: fac.ht, tva: fac.tva, ttc: fac.ttc, devise: fac.devise || 'MAD',
    montant_recu: fac.montant_recu || 0, showStamp: fac.statut === 'payee', doc_id: fac.id,
...
async function ajouterEntrepriseCommeClient(entrepriseId) {
  try {
    const resp = await fetch(SUPABASE_URL + '/rest/v1/profils_entreprise?id=eq.' + entrepriseId + '&select=*', { headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + sb.token } });
    const profils = await resp.json() || [];
    const profil = profils[0];
    if (!profil) return;
    const existResp = await fetch(SUPABASE_URL + '/rest/v1/clients?user_id=eq.' + (STATE.entrepriseId || sb.user.id) + '&reference_id=eq.' + entrepriseId + '&limit=1', { headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + sb.token } });
    const exist = await existResp.json() || [];
    if (exist.length > 0) return;
    await fetch(SUPABASE_URL + '/rest/v1/clients', {
      method: 'POST',
      headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + sb.token, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
      body: JSON.stringify({
        user_id: (STATE.entrepriseId || sb.user.id), nom: profil.raison || profil.email || '', email: profil.email || '',
        tel: profil.tel || '', adresse: profil.adresse || '', ville: profil.ville || '', ice: profil.ice || '', rc: profil.rc || '',
        note: 'Client Zelto · ' + (profil.secteur || ''), reference_id: entrepriseId, type: 'entreprise_banipay'
      })
    });
  } catch(e) {}
}
...
function attacherControlsToViewer(factureId) {
  ...
  // FIX (grand audit - correctif 1): les 2 boutons n'avaient jamais leur
  // etat initial (data-lettre/data-tva) - toggleLettrageRapide()/
  // toggleTVARapide() lisent justement cet attribut pour savoir s'il faut
  // cocher ou decocher. Sans lui, undefined !== '1' est toujours vrai:
  // le premier clic sur une facture DEJA lettree la "relettrait" au lieu
  // de la de-lettrer (comportement inverse), un second clic etait
  // necessaire pour revenir a un comportement normal.
  bar.innerHTML =
    '<button class="btn-l-ctrl" data-fid="' + factureId + '" data-lettre="' + (ctrl.lettre ? '1' : '0') + '" style="width:44px;height:44px;border-radius:10px;border:none;background:' + (ctrl.lettre ? '#6E8F4E' : '#EAE4DA') + ';color:' + (ctrl.lettre ? '#fff' : '#9C9186') + ';font-size:18px;font-weight:900;cursor:pointer;font-family:inherit">L</button>' +
  ...
  bar.querySelector('.btn-l-ctrl').onclick = async function() {
    ...
    if (txt) txt.textContent = this.dataset.lettre === '1' ? '✓ Lettre' : 'Non lettre';
  };
  bar.querySelector('.btn-t-ctrl').onclick = async function() {
    ...
    if (txt) txt.textContent = this.dataset.tva === '1' ? '✓ TVA ok' : 'TVA non verifiee';
  };
